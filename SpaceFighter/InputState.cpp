/* ------------------------------------------------
 ...
 InputState.h: Header file for checking the
 current input state.
...
void InputState::InitializeGamePads()
{
	for (int i = 0; i < al_get_num_joysticks(); i++)
	{
		...
		if (strcmp(al_get_joystick_name(joystick), "Controller (XBOX 360 For Windows)") == 0)
		{
			for (int i = 0; i < MAX_NUM_GAMEPADSTATES; i++)
			{
				if (!m_currentGamePadStates[i].ID)
				{
					...
					//std::cout << al_get_joystick_name(joystick) << " mapped to state " << i << std::endl;
...
bool InputState::IsKeyDown(int const key) const
{
	return al_key_down(&m_currentKeyboardState, key);
}

bool InputState::IsKeyUp(int const key) const
{
	return !al_key_down(&m_currentKeyboardState, key);
}

bool InputState::IsNewKeyPress(int const key) const
{
...
GamePadState &InputState::GetGamePadState(int const gamePadIndex)
{
